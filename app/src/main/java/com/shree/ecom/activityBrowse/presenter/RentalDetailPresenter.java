...
public class RentalDetailPresenter implements RentalDetailContract.Presenter {
    private static final int API_STATUS_CODE_LOCAL_ERROR = 0;
    private RentalDetailContract.View view;
    private RentalDetailContract.Model model;
    private Disposable disposable;

    public RentalDetailPresenter(RentalDetailContract.Model model) {
        this.model = model;
    }

    @Override
    public void setView(RentalDetailContract.View view) {
        this.view = view;
    }
    ...
    public void loadData() {
        ...
        disposable = model
                ...
                .subscribeWith(new DisposableObserver<RentalEquipmentDetailEntity>() {
                    @Override
                    public void onNext(RentalEquipmentDetailEntity rentalEquipmentDetailEntity) {
                        if (view != null) {
                            view.progressOn(false);
                            List<RentalDetailDataDto> rentalDetailDataDtoList = new ArrayList<>();
                            for (int i = 0; i < rentalEquipmentDetailEntity.getData().size(); i++) {
                                rentalDetailDataDtoList.add(rentalEquipmentDetailEntity.getData().get(i));
                            }
                            view.loadData(rentalDetailDataDtoList);
                        } else {
